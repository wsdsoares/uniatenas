    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        LISTA DE <?php echo 'PROVAS E GABARITOS'; ?>
                    </h2>
                    <?php
                    $page = $this->uri->segment(2);
                    if (in_array("regProvasGab", $permissionCampusArray['campus-1'])) {
                        echo anchor('Painel_vestibular/cadastrar_provaGab', '<i class="material-icons">add_box</i> CADASTRAR ', array('class' => 'btn btn-primary m-t-15 waves-effect'));
                    }?>
                            ...
                            foreach ($listagem as $item):
                                ...
                                <tr>
                                    <td class="center">
                                        ...
                                        if (in_array("regProvasGab", $permissionCampusArray['campus-1'])) {
                                            echo '<a href=' . base_url("Painel_vestibular/editar_provaGab/$item->id") . '>'
                                                . '<i class="material-icons">edit</i>'
                                                . '</a> ';
                                            if ($item->status == 1) {
                                                echo '<a href=' . base_url("Painel_vestibular/statusAlter_provaGab/$item->id/0") . '>'
                                                    . '<i class="material-icons">visibility</i>'
                                                    . '</a>';
                                            } elseif ($item->status == 0) {
                                                echo '<a href=' . base_url("Painel_vestibular/statusAlter_provaGab/$item->id/1") . '>'
                                                    . '<i class="material-icons">visibility_off</i>'
                                                    . '</a>';
                                            }
                                        }
                                        if(in_array("admProvaGab", $permissionCampusArray['campus-1'])){
                                            echo '<a href=' . base_url("Painel_vestibular/delete_tempArg/$item->id") . '>'
