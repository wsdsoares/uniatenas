<style>
    .sectionFinanciamentos {
        margin-top: 2%;

    }

    .sectionFinanciamentos .row {
        margin-bottom: 2%;
    }

    .sectionFinanciamentos .row .col-md-6 {
        background: #f5f5f5;
        padding: 2%;
    }

    .sectionFinanciamentos-header {
        text-align: center;
    }

    .sectionFinanciamentos h3 {
        color: #004085;
    }

    .sectionFinanciamentos p.text {
        margin-top: 6%;
        color: #545b62;
    }

    .sectionFinanciamentos img {
        width: 100%;
        max-height: 260px;
    }

    .imageFinanciamento, .textFinanciamento {
        max-height: 260px;
    }

    .descriptionFinanciamentos {
        padding: 10px 0 30px 0;
    }

    .sectionFinanciamentos a.linksFinanceiro {
        margin-top: 10px;
    }
</style>


<?php


$campus = $this->uri->segment(3);

echo '<br>';
echo '<br>';
echo '<br>';
echo '<br>';
echo '<pre>';
print_r($dados);
echo '<pre>';
?>

<div class="container sectionFinanciamentos">
    <div class="sectionFinanciamentos-header">
        <h3>
            Financeiro <?php echo $dados['campus']->name . ' - ' . $dados['campus']->city . ' (' . $dados['campus']->uf . ')'; ?></h3>
    </div>
    <div class="row">
        <?php if($camp->id == 3 || $camp->id == 1):?>
        <div class="col-md-9">
            <?php endif; ?>
            <div class="descriptionFinanciamentos">
                <span><?php echo $conteudoPag[0]->description; ?> </span>
            </div>
            <?php

            for ($i = 1; $i < count($dados['conteudoPag']); $i++) {

                ?>
                <div class="row">
                    <div class="col-md-6 textFinanciamento text-justify">
                        <h3>
                            <?php echo $dados['conteudoPag'][$i]->title; ?>
                        </h3>
                        <p class="text">
                            <?php
                            $max = 150;
                            $str = $dados['conteudoPag'][$i]->description;
                            to_html(substr_replace($str, (strlen($str) > $max ? '...' : ''), $max));

                            echo $texto = mb_substr($str, 0, mb_strrpos(mb_substr($str, 0, $max), ' '), 'UTF-8') . '...';
                            ?>
                        </p>
                        <div class="text-right">
                            <?php echo anchor('financeiro/pagina/' . $campus . '/' . $dados['conteudoPag'][$i]->id, '<i class="fa fa-plus"></i> Informações', array('class' => 'btn btn-success linksFinanceiro')); ?>
                        </div>
                    </div>
                    <div class="col-md-5 imageFinanciamento">
                        <?php echo anchor('financeiro/pagina/' . $campus . '/' . $dados['conteudoPag'][$i]->id, "
                <img src='" . base_url($dados['conteudoPag'][$i]->img_destaque) . "'/>");
                        ?>
                    </div>
                </div>
                <?php
            }
            ?>
        </div>
        <label>

        </label>
        <?php
         if($camp->id == 3 || $camp->id == 1):?>
        <div class="col-md-3">
            <?php if (!empty($contatos)): ?>
                <div class="widget-sidebar">
                    <h2 class="title-widget-sidebar">Contatos</h2>
                    <div class="content-widget-sidebar">
                        <ul>
                            <li class="recent-post-alunos">
                                <div class="col-sm-2 col-xs-2">
                                    <i class="far fa-address-card fa-2x"></i>
                                    </a>
                                </div>
                                <div class="col-sm-12 col-xs-12 ">
                                    <?php foreach ($contatos as $phone): ?>
                                        <small>
                                            <div class=" ">
                                                <?php if ($phone->email != "" || $phone->email != null): ?>
                                                    <p>
                                                        <label>E-mail:</label><br>
                                                    <p>
                                                        <text class="Orange-label"><?= $phone->email ?></text>
                                                    </p>
                                                    </p>
                                                <?php endif; ?>
                                                <?php if ($phone->phonesetor != "" || $phone->phonesetor != null): ?>
                                                    <p class="Orange-label">
                                                        <label>Telefone: </label><br>
                                                        <text class="Orange-label"><?= $phone->phonesetor ?></text>
                                                    </p>
                                                <?php endif; ?>
                                                <?php if ($phone->ramal != "" || $phone->ramal != null): ?>
                                                    <p>
                                                        <label>Telefone: </label><br>
                                                        <text class="Orange-label"><?= $camp->phone; ?></text>
                                                    </p>
                                                    <p>
                                                        <label>Ramal: </label><br>
                                                        <text class="Orange-label"><?= $phone->ramal; ?></text>
                                                    </p>
                                                <?php endif; ?>
                                                <?php if ($phone->phone != "" || $phone->phone != null): ?>
                                                    <p>
                                                        <label>Cel. Corporativo:</label><br>
                                                        <text class="Orange-label"><?= $phone->phone; ?></text>
                                                    </p>
                                                    <p>
                                                        <label>WhatsApp Business:</label><br>
                                                        <text class="Orange-label">(38) 3365-1036</text>
                                                        <label>Horário de Atendimento:</label><br>
                                                        <text class="Orange-label">08:00 às 21:30</text>
                                                    </p>
                                                <?php endif; ?>
                                            </div>
                                        </small>
                                    <?php
                                    endforeach;
                                    ?>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            <?php endif; ?>
        </div>
        <?php
     endif; ?>
    </div>
</div>
