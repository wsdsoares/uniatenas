...
class Vestibular extends CI_Controller
{
    ...
    public function resultado_geral()
    {
        ...
        if ($idCampus == 1) {
            $tabela = 'paracatu_2020_resultados';
            $resultadosOficiais = count($this->bancoVestibular->getWhere($tabela)->result());
        } elseif ($idCampus == 2) {
            $tabela = 'setelagoas_2020_resultados';
            $resultadosOficiais = count($this->bancoVestibular->getWhere($tabela)->result());
        } elseif ($idCampus == 3) {
            $tabela = 'passos_2020_resultados';
            $resultadosOficiais = count($this->bancoVestibular->getWhere($tabela)->result());
        }
        ...
        $whereEssay = array($tabela, $tabela . '.ano' => '2019', $tabela . '.' . $fieldQuery => $infCandidato);
        ...
        $data = array(
            'head' => array(
                'title' => 'Vestibular 2020 - Medicina',
            ),
    ...
    public function resultado()
    {
        $data = array(
            'head' => array(
                'title' => 'Vestibular 2019 - UniAtenas',
            ),
    ...
    public function buscaResultado($campo = NULL, $tabela = NULL)
    {
        if ($campo) {
            if ((strlen($campo) == 11)) {
                ...
                $retorno['resultado'] = $this->bancoVestibular->getQuery("Select * from paracatu_2020_resultados where cpf = $campo")->row();
                $retorno['tipo'] = 'cpf';
                //$this->resultFinal('1', $retorno['resultado']->cpf  );
            } elseif (strlen($campo) == 8) {

                $retorno['resultado'] = $this->bancoVestibular->getQuery("Select * from paracatu_2020_resultados where where inscricao = $campo")->row();
