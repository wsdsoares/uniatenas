...
class Painel_secretaria Extends CI_Controller
{
    ...
    public function calendarios_semestre($campus = null)
    {
        switch ($campus) {
            case 1:
                $namecity = 'Paracatu';
        ...
        $data = array(
            ...
            'conteudo' => 'paineladm/secretaria/calendariosSemestre/lista',
            'dados' => array(
    ...
    public function cadastrar_calendariosSemestre($campus = null, $table = null)
    {
    ...
    public function editar_calendariosSemestre($campus = null, $id = NULL, $table = null)
    {
        $this->load->helper('file');
        verificaLogin();
        $tableD = $table;
        ...
        $table = 'campus_calendars';
        $datajoin = array('campus' => 'campus_calendars.campusid = campus.id ');
        $where = array('campusid' => $campus, 'campus_calendars.id' => $id);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($listagem->status != $this->input->post('status')-1) {
                $dados_form['status'] = $this->input->post('status')-1;
            }
            ...
            } elseif (isset($dados_form['name']) || isset($dados_form['semester']) || isset($dados_form['year']) || isset($dados_form['status'])||isset($dados_form['type'])) {
                if ($this->bd->salvar($table, $dados_form) == TRUE) {
        ...
        //devolvendo o formulario
        $data = array(
            'titulo' => "Editar Calendadario  - $namecity",
            'conteudo' => 'paineladm/secretaria/calendariosSemestre/editar',
            'dados' => array(
                'permissionCampusArray' => $_SESSION['permissionCampus'],
                'id' => $id,
                'idtipo'=> $idtipo,
